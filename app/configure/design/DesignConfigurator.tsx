...
const {mutate:saveConfig,isPending}=useMutation({
    ...
    onError:()=>{
        toast({
            ...
            description:'There was as Error on Our end. Please try again.',
            variant:'destructive'
        })
    },
    onSuccess:()=>{
...
async function saveConfiguration(){
    ...
    const acutalY=renderedPosition.y-topOffset;
...
return <div className="relative mt-20 grid grid-cols-1 lg:grid-cols-3 mb-20 pb-20 ">
    <div  ref={containerRef} className="relative h-[37.5rem] overflow-hidden col-span-2 w-full max-w-4xl flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 p-12 text-center focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 ">
        <div className="relative w-60 bg-opacity-50 pointer-events-none aspect-[896/1831] ">
            ...
            <div className={clsx('absolute inset-0 left-[3px] top-px right-[3px] bottom-px rounded-[32px] ',
                options.color.tw === 'zinc-900' && 'bg-zinc-900',
              options.color.tw === 'blue-950' && 'bg-blue-950',
              options.color.tw === 'rose-950' && 'bg-rose-950'
             )}/>
        </div>
       <Rnd default={{
        x:150,
    ...
    <div className="h-[37rem] flex flex-col  w-full col-span-full lg:col-span-1  bg-white ">
        <ScrollArea  className="relative flex-1 overflow-auto ">
            <div  aria-hidden="true"  className="absolute z-50 inset-x-0 bottom-0 h-12 bg-gradient-to-t from-white  pointer-events-none "/>
                    ...
                    <RadioGroup value={options.color} onChange={(val)=>{
                        ...
                        <div className="mt-3 flex items-center space-x-3 ">
                            {COLORS.map((color)=>(
                                ...
                                className={({active,checked})=>clsx('relative -m-0.5 flex cursor-pointer items-center justify-center rounded-full p-0.5  active:ring-0 focus:ring-0 active:outline-none focus:outline-none border-2 border-transparent',
                                    {
                                        "border-zinc-900": (active || checked) && color.tw === "zinc-900",
                                        "border-blue-950": (active || checked) && color.tw === "blue-950",
                                        "border-rose-950": (active || checked) && color.tw === "rose-950",
                                        "border-transparent": !(active || checked),
                                    }
                                )}
                                >
                                    <span className={clsx(  color.tw === 'zinc-900' && 'bg-zinc-900',
                          color.tw === 'blue-950' && 'bg-blue-950',
                          color.tw === 'rose-950' && 'bg-rose-950',"h-8 w-8 rounded-full border border-black  border-opacity-10   ")}/>
                                </RadioGroup.Option>
                            ))}
                        </div>
                                    ...
                                    <RadioGroup.Option key={option.value} value={option} className={({active,checked})=>cn('relative block cursor-pointer rounded-lg  bg-white px-6 py-4 shadow-sm border-2 border-zinc-200 focus:outline-none ring-0 focus:ring-0 outline-none sm:flex sm:justify-between',{
                                        ...
                                        <span className="flex items-center ">
                                            <span className="flex flex-col text-sm ">
                                                ...
                                                {option.description? <RadioGroup.Description as="span" className="text-gay-500">
                                                    <span className=" mt-2 block sm:inline">{option.description}</span>
        ...
        <div className="w-full px-8 h-16 bg-white ">
            ...
            <div className="w-full h-full flex justify-end items-center ">
                <div className="w-full flex gap-6 items-center ">
                    ...
                    <Button isLoading={isPending} disabled={isPending} loadingText="saving" onClick={()=>saveConfig({
