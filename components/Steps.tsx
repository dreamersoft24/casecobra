...
const Steps =()=>{
    ...
    return <ol className="rounded-md bg-white 
    lg:flex lg:rounded-none lg:border-1 lg:border-r lg:border-gray-200">
        {STEPS.map((step,i)=>{
        const isCurent =pathname.endsWith(step.url);
            ...
            return <li key={step.name} className="relative overflow-hidden lg:flex-1">
                <div>
                    <span className={cn('absolute left-0 top-0 h-full w-1 bg-zinc-400 lg:bottom-0 lg:top-auto lg:h-1 lg:w-full',{
                        'bg-zinc-700':isCurent,
                        'bg-primary':isCompleted
                    })} aria-hidden="true"/>

                <span className={cn(i===0?'lg:pl-9':'','flex items-center px-6 py-4 text-sm font-medium')}>
                    <span className="flex-shrink-0 ">
                        <Image src={imgPath} alt="" width={400} height={400}  className={cn('flex h-20 w-20 object-contain items-center justify-center',{
                            "border-none":isCompleted,
                            'border-zinc-700':isCurent,
                        })}/>
                    </span>
